...
Variable M : gen_RMonad_Hom_struct.

Program Instance gen_RMonad_Hom_NatTrans : 
...
Proof.
  ...
  intros c d f.
  unfold rlift.
  assert (H':=gen_rmonad_hom_rkl (gen_RMonad_Hom_struct := M)).
  rewrite H'.
  apply praecomp.
  apply rkleisli_oid.
  assert (HN:=NTcomm N).
...
*)  
  ...
  rewrite assoc.
  rewrite FComp.
  assert (H:= gen_rmonad_hom_rweta (gen_RMonad_Hom_struct := M)).
  rewrite <- H.
...
Definition gen_RMonad_Hom_NT := Build_NT gen_RMonad_Hom_NatTrans.

End Monad_Hom_NT.

End gen_RMonad_Hom_struct.

Record gen_RMonad_Hom := {
(*  Gone : Functor C C' ;
  Gtwo : Functor D D' ;*)
(*  GOGT : NT (CompF G1 F') (CompF F G2) ;*)
  gen_rmonad_hom :> forall (c : obC), morD' (G2 (P c)) (Q (G1 c)) ;
  gen_rmonad_hom_struct :> gen_RMonad_Hom_struct  gen_rmonad_hom
}.
...
Program Instance gen_RMonad_Hom_id_s : gen_RMonad_Hom_struct 
     (P:=P)(Q:=P)(G1:=Id _ )(G2:=Id _ )
     (Build_NT NT_i)
     (fun c => id _ ).

Definition gen_RMonad_Hom_id :=
    Build_gen_RMonad_Hom gen_RMonad_Hom_id_s.
...
Variable S : gen_RMonad_Hom P Q NS.
Variable T : gen_RMonad_Hom Q R NTT.
...
Program Instance gen_RMonad_Hom_comp_s : gen_RMonad_Hom_struct
  (P:=P) (Q:=R) (G1:=CompF 
                      (G1S)
                      (G1T)) 
                (G2:=CompF 
...
Next Obligation.
Proof.
  assert (H:=gen_rmonad_hom_rweta (gen_RMonad_Hom_struct := T)).
  assert (H':=gen_rmonad_hom_rweta (gen_RMonad_Hom_struct := S)).
  rewrite <- H.
  repeat rewrite  assoc.
  apply praecomp.
  rewrite <- H'.
...
Next Obligation.
Proof.
  assert (H:=gen_rmonad_hom_rkl (gen_RMonad_Hom_struct := T)).
  assert (H':=gen_rmonad_hom_rkl (gen_RMonad_Hom_struct := S)).
  rewrite <- assoc.
  rewrite <- assoc.
  rewrite <- FComp.
  rewrite  H'.
...
Definition gen_RMonad_Hom_comp :=
    Build_gen_RMonad_Hom gen_RMonad_Hom_comp_s.
...
Definition gen_PbRMod := Build_RModule gen_PbRMod_s.
...
Variable f : gen_RMonad_Hom P Q T.
...
Program Instance gen_PbRMod_ind_Hom_s : RModule_Hom_struct
    (M:=F_rmod) (N:=gen_PbRMod f Q) f.
Next Obligation.
Proof.
  simpl.
  intros c d g.
...
Definition gen_PbRMod_ind_Hom := 
   Build_RModule_Hom gen_PbRMod_ind_Hom_s.
...
Program Instance gen_PROD_PM_s : RModule_Hom_struct 
        (M:=product (C:=RMOD P E) (gen_PbRMod f N) (gen_PbRMod f N'))
        (N:=gen_PbRMod f (product (C:=RMOD Q E) N N'))
        (fun c => id (product (N (G1 c)) (N' (G1 c)))).

Definition gen_PROD_PM : 
     product (C:=RMOD P E) (gen_PbRMod f N) (gen_PbRMod f N') ---> 
            gen_PbRMod f (product (C:=RMOD Q E) N N') :=
      Build_RModule_Hom gen_PROD_PM_s.

Variable h : N ---> N'.

Obligation Tactic := rmonad.

Program Instance gen_PbRMod_Hom_s : RModule_Hom_struct 
     (M:=gen_PbRMod f N) (N:=gen_PbRMod f N') (fun c => h (G1 c)).

Definition gen_PbRMod_Hom : gen_PbRMod f N ---> gen_PbRMod f N' :=
       Build_RModule_Hom gen_PbRMod_Hom_s.

End pb_prod_func_and_Pb_ind_Hom.

Coercion gen_RMonad_Hom_NatTrans : gen_RMonad_Hom_struct >-> NT_struct.
Coercion gen_RMonad_Hom_NT : gen_RMonad_Hom_struct >-> NT.
