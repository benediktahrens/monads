...
Definition wPO := Build_Cat wPO_struct.

Class Monotone (a b : wPO) (f : a ---> b) := 
...
*)
Program Instance wPO_monotone_SubCat : SubCat_compat wPO
      (fun x => True) (fun a b f => Monotone f).

Definition wPO_monotone := SubCat wPO_monotone_SubCat.

Program Instance wPO_prod : Cat_Prod (wPO) := {
...
Program Instance wPO_Terminal : Terminal wPO := {
  Term := PO_TERM;
  TermMor a := fun x => tt
}.
...
Program Instance TYPE_init : Initial wPO := {
  Init := PO_INIT 
}.
