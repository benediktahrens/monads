...
Require Export CatSem.COMP.PCF_rep_hom_quant.
...
Program Instance Rep_id_struct : 
         ...
         (fun t => eq_refl) (fun u v => eq_refl) id_Rep_monad. 
Next Obligation.
Proof.
  simpl.
  intros V y.
  assert (H := mod_hom_mkl 
      (Module_Hom_struct :=(Succ (PCF_rep_struct:=P)))).
  unfold lift.
  simpl in *.
  rewrite <- H.
  auto.
...
Proof.
  ...
  intros V y.
  assert (H := mod_hom_mkl 
      (Module_Hom_struct :=(Zero(PCF_rep_struct:=P)))).
  unfold lift.
  simpl in *.
  rewrite <- H.
  auto.
...
Proof.
  ...
  intros V y.
  assert (H := mod_hom_mkl 
      (Module_Hom_struct :=(CondB(PCF_rep_struct:=P)))).
  unfold lift.
  simpl in *.
  rewrite <- H.
  auto.
...
Proof.
  ...
  intros V y.
  assert (H := mod_hom_mkl 
      (Module_Hom_struct :=(CondN(PCF_rep_struct:=P)))).
  unfold lift.
  simpl in *.
  rewrite <- H.
  auto.
...
Proof.
  ...
  intros V y.
  assert (H := mod_hom_mkl 
      (Module_Hom_struct :=(tttt(PCF_rep_struct:=P)))).
  unfold lift.
  simpl in *.
  rewrite <- H.
  auto.
...
Proof.
  ...
  intros V y.
  assert (H := mod_hom_mkl 
      (Module_Hom_struct :=(ffff(PCF_rep_struct:=P)))).
  unfold lift.
  simpl in *.
  rewrite <- H.
  auto.
...
Proof.
  ...
  intros m V y.
  assert (H := mod_hom_mkl 
      (Module_Hom_struct :=(nats m(PCF_rep_struct:=P)))).
  unfold lift.
  simpl in *.
  rewrite <- H.
  auto.
...
Proof.
  ...
  intros t V y.
  assert (H := mod_hom_mkl 
      (Module_Hom_struct :=(bottom t(PCF_rep_struct:=P)))).
  unfold lift.
  simpl in *.
  rewrite <- H.
  auto.
...
Proof.
  ...
  intros t V y.
  assert (H := mod_hom_mkl 
      (Module_Hom_struct :=(rec t(PCF_rep_struct:=P)))).
  unfold lift.
  simpl in *.
  rewrite <- H.
  auto.
...
Proof.
  ...
  intros r s V y.
  assert (H := mod_hom_mkl 
      (Module_Hom_struct :=(app(PCF_rep_struct:=P) r s ))).
  unfold lift.
  simpl in *.
  rewrite <- H.
  auto.
...
Proof.
  ...
  intros r s V y.
  assert (H := mod_hom_mkl 
     (Module_Hom_struct :=(abs(PCF_rep_struct:=P) r s))).
  unfold lift.
  simpl in *.
  rewrite <- H.
  apply f_equal.
...
Proof.
  ...
  intros V y.
  assert (H := mod_hom_mkl 
      (Module_Hom_struct :=(Pred(PCF_rep_struct:=P)))).
  unfold lift.
  simpl in *.
  rewrite <- H.
  auto.
