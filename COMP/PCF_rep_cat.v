...
Require Export CatSem.COMP.PCF_rep_comp.
...
Program Instance REP_s : 
     Cat_struct (obj := PCF_rep) (PCF_rep_Hom) := {
...
Next Obligation.
  ...
  assert (H3:=MonadHomLift rep_Hom_monad).
  assert (H4:=H3 _ _ t y (id_retype (V:=V))).
  simpl in H4.
  rewrite <- H4.
  clear H4.
  (*
  rewrite <- (H3 _ _ t y (id_retype (V:=V))).
