...
Require Export CatSem.PCF_o_c.ULC_syntax.
...
Definition App_prep u v c:
(PO_product (ipo_proj (ULCBETA c) tt) (ipo_proj (ULCBETA c) u) ->
  ipo_proj (ULCBETA c) v).
intro u.
destruct u.
intro v.
destruct v.
intro c.
simpl.
apply (fun y => App (fst y) (snd y)).
Defined.
Print App_prep.
Definition App_prep2 u v c:
(PO_product (ipo_proj (ULCBETA c) tt) (ipo_proj (ULCBETA c) u) ->
  ipo_proj (ULCBETA c) v).
simpl; intros u v c y.
destruct u.
destruct v.
apply (App (fst y) (snd y)).
Defined.
Print App_prep.
Print App_prep2.
...
Definition Abs_prep u v c :
 ipo_proj (ULCBETA (opt u c)) v -> ipo_proj (ULCBETA c) tt.
simpl.
intros u v c y.
destruct u.
destruct v.
...
Definition Rec_prep V t:
(ipo_proj (ULCBETA V) tt -> ipo_proj (ULCBETA V) t).
simpl.
intros V t y.
destruct t.
apply (App (ULC_theta _ ) y).
Defined.
...
(App 
     (App
        (Abs
           (Abs
      (Var (V:=(V *) *) (some tt (A:=V *) (t:=tt) (none tt V)))
)) n) m).
apply cp_App1.
apply cp_App1.
apply cp_Abs.
...
(Abs (Abs (Var (V:=(V *) *) (none tt (opt tt V))))))
.
unfold substar in H.
simpl in H.
unfold inj in H.
...
(App 
     (App 
        (Abs
           (Abs
              (App 
                 (Abs (Var (V:=((V *) *) *) (none tt (V *) *)))
                 (Var (V:=(V *) *) (none tt (opt tt V)))))) n) m)
).
apply cp_App1.
apply cp_App1.
apply cp_Abs.
...
assert (H:= app_abs 
  (Var (V:=((V *) *) *)  (none tt (opt tt (opt tt V ) )))
  (Var (V:=(V *) *) (none tt V *))
).
unfold substar in H;
simpl in H.
