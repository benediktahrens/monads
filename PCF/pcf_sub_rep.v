...
Notation "y [* := z ]":= (IOsubstar z  y)(at level 55).
...
Class PCFPO_rep_struct (P : Monad IP) := {
  ...
  beta: forall r s V y z, 
        App r s V (Abs r s V y, z) <<  (*IPsubstar z _ y*) y[*:= z] ;
...
Lemma id_Term_PB_struct: 
   Module_Hom_struct (S:= Term (C:= MOD _ SO))  (T:=(PB_MOD Sig SO Term))  
                 (fun r => id SO_TERM).
Proof. 
  constructor.
  intros.
...
Definition PBT : Term ---> (PB_MOD Sig SO Term) :=
