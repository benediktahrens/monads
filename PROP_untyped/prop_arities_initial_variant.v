...
Class half_equation_struct (U V : S_Module) 
  ...
  comm_eq_s : forall (R T : REP S)  (f : R ---> T), 
      s_mod_hom U (repr_hom_c f) ;; PbRMod_Hom _ (half_eq T) ==  
            half_eq R ;; s_mod_hom V (repr_hom_c f) }.
...
Definition subst_carrier (P : RMonad Delta) :
...
Program Instance sub_struct (P : RMonad Delta) : RModule_Hom_struct 
...
Definition subst_module_mor (P : RMonad Delta) := Build_RModule_Hom (sub_struct P).


Program Instance subst_half_s : half_equation_struct 
      (U:= (S_Mod_classic [[1 ; 0]])) (V:=S_Mod_classic [[0]]) (fun R => subst_module_mor R).
Next Obligation.
Proof.
...
(*
  ...
  simpl in *|-*.
  intros.
  rewrite sts_list_subst in H'.
  unfold prod_mod_c_rel in H'.
  unfold pm_f_UTSl in H.
  simpl in H.
  apply .
